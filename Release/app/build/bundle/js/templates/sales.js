angular.module('myShopApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('bills.list.html',
    "<div class=\"panel panel-red\"><div class=\"panel-heading\"><h3 class=\"panel-title\"><i class=\"fa fa-list\"></i>&nbsp; Bills</h3></div><div class=\"panel-body\"><div class=\"row\"><div class=\"col-lg-5 pull-right\"><div class=\"form-group input-group\"><input type=\"text\" class=\"form-control\" ng-model=\"vm.searchText\"> <span class=\"input-group-addon\"><i class=\"fa fa-search\"></i></span></div></div></div><div class=\"row\"><div class=\"col-lg-12\"><div class=\"table-responsive\"><table class=\"table table-bordered table-hover table-striped\"><thead><tr><th>Customer</th><th>Amount</th><th>Bill Date</th><th>Created On</th><th>Modified On</th><th></th></tr></thead><tbody><tr><td colspan=\"4\" class=\"text-center\" ng-if=\"filteredBill.length === 0\"><span class=\"text-danger\">No Customer Found</span></td></tr><tr ng-repeat=\"bill in filteredBill = (vm.bills | filter : vm.searchText) | limitTo:5:5*(vm.page-1)\"><td ng-bind=\"bill.customer.name\"></td><td ng-bind=\"bill.grandTotal\"></td><td ng-bind=\"bill.billDate | date:'dd-MM-yyyy hh:mm a'\"></td><td ng-bind=\"bill.createdOn | date:'dd-MM-yyyy hh:mm a'\"></td><td ng-bind=\"bill.modifiedOn | date:'dd-MM-yyyy hh:mm a'\"></td><td><button type=\"button\" uib-tooltip=\"Edit\" class=\"btn btn-danger btn-circle\" ng-click=\"vm.editBill(bill._id)\"><i class=\"fa fa-pencil-square-o\" aria-hidden=\"true\"></i></button> <button type=\"button\" uib-tooltip=\"View\" class=\"btn btn-danger btn-circle\" ng-click=\"vm.viewBill(bill._id)\"><i class=\"fa fa-eye\" aria-hidden=\"true\"></i></button></td></tr></tbody></table><ul uib-pagination class=\"pull-right red-pagination pagination-sm pagination\" total-items=\"filteredBill.length\" ng-model=\"vm.page\" previous-text=\"&lsaquo;\" next-text=\"&rsaquo;\" items-per-page=\"5\"></ul></div></div></div></div></div>"
  );


  $templateCache.put('bills.view.html',
    "<div class=\"panel panel-red\"><div class=\"panel-heading\"><h3 class=\"panel-title\"><i class=\"fa fa-list\"></i>&nbsp; Bills</h3></div><div class=\"panel-body\"><div print-section><div class=\"row padding-bottom-25\"><div class=\"col-lg-12 col-md-12 col-sm-12 text-right\"><!--<h2>मे. रमेशचंद्र हिरालाल झंवर</h2>--><h1>M/s. Rameshchandra Hiralal Jhawar</h1><div class=\"text-right\"><strong>245/908 , Adat Bazar, Ahmednagar, Maharashtra, 414001</strong></div><div class=\"text-right\"><i class=\"fa fa-mobile\" aria-hidden=\"true\"></i> : <strong>9253380029</strong> &nbsp; <i class=\"fa fa-phone\" aria-hidden=\"true\"></i> : <strong>0241-2345037</strong></div></div></div><div class=\"row padding-bottom-25 client-info\" print-remove><div class=\"col-lg-12 col-md-12 col-sm-12\"><h4><strong>Customer Information</strong></h4><strong ng-bind=\"vm.billDetails.customer.name\"></strong><div ng-if=\"vm.billDetails.customer.address\"><span ng-bind=\"vm.billDetails.customer.address\"></span></div><div ng-if=\"vm.billDetails.customer.mobileNumber || vm.billDetails.customer.landLineNumber\"><i class=\"fa fa-mobile\" aria-hidden=\"true\"></i> : <strong ng-bind=\"vm.billDetails.customer.mobileNumber ||'-'\"></strong>&nbsp; <i class=\"fa fa-phone\" aria-hidden=\"true\"></i> : <strong ng-bind=\"vm.billDetails.customer.landLineNumber.stdCode +' - '+vm.billDetails.customer.landLineNumber.number\"></strong></div></div><!--<div class=\"col-lg-6 col-md-6 col-sm-6\">\r" +
    "\n" +
    "                    <h4> <strong>Payment Details </strong></h4>\r" +
    "\n" +
    "                    <b>Bill Amount :  <i class=\"fa fa-inr\"></i>&nbsp;{{vm.billDetails.grandTotal}} </b>\r" +
    "\n" +
    "                    <br /> Bill Date : {{vm.billDetails.billDate | date:'dd-MMMM-yyyy'}}\r" +
    "\n" +
    "                    <br /> <b>Payment Status :  Paid </b>\r" +
    "\n" +
    "                    <br /> <b>Payment Mode :  Cash </b>\r" +
    "\n" +
    "                </div>--></div><div class=\"row\"><div class=\"col-lg-12 col-md-12 col-sm-12\"><div class=\"table-responsive\"><table print-table=\"vm.billDetails.billItems\" class=\"table table-bordered\"><thead><tr><th style=\"width:5%\">#</th><th style=\"width:20%\">Product</th><th style=\"width:15%\">Brand</th><th style=\"width:10%\">Units</th><th style=\"width:15%\">Rate / kg.</th><th style=\"width:20%\">Weight (kgs.)</th><th style=\"width:15%\">Total</th></tr></thead><tbody><tr ng-repeat=\"item in vm.billDetails.billItems\"><td ng-bind=\"$index + 1\"></td><td ng-bind=\"item.product.name\"></td><td ng-bind=\"item.brand\"></td><td ng-bind=\"item.units\"></td><td ng-bind=\"item.rate\"></td><td><span class=\"weight\" ng-repeat=\"weight in item.weights track by $index\"><span ng-bind=\"weight\"></span> <span ng-if=\"item.weights.length > 1 && !$last\">,</span></span></td><td><i class=\"fa fa-inr\"></i>&nbsp;<label ng-bind=\"item.total\"></label></td></tr><tr class=\"success\"><td colspan=\"6\" class=\"text-right\"><label>Sub Total</label></td><td><i class=\"fa fa-inr\"></i>&nbsp;<label ng-bind=\"vm.billDetails.subTotal\"></label></td></tr><!--<tr class=\"warning\">\r" +
    "\n" +
    "                                    <td colspan=\"5\" class=\"text-right\"><label>Labour Charge </label></td>\r" +
    "\n" +
    ...
    "                                <tr class=\"warning\">\r" +
    "\n" +
    "                                    <td colspan=\"5\" class=\"text-right\"><label>Lavy </label></td>\r" +
    "\n" +
    "                                    <td><i class=\"fa fa-inr\"></i>&nbsp;<label ng-bind=\"vm.billDetails.lavy\"></label></td>\r" +
    "\n" +
    "                                </tr>\r" +
    "\n" +
    "                                <tr class=\"warning\">\r" +
    "\n" +
    "                                    <td colspan=\"5\" class=\"text-right\"><label>Transport Charge </label></td>\r" +
    "\n" +
    "                                    <td><i class=\"fa fa-inr\"></i>&nbsp;<label ng-bind=\"vm.billDetails.transportCharge\"></label></td>\r" +
    "\n" +
    "                                </tr>\r" +
    "\n" +
    "                                <tr class=\"warning\">\r" +
    "\n" +
    "                                    <td colspan=\"5\" class=\"text-right\"><label>VAT/CST  </label></td>\r" +
    "\n" +
    "                                    <td><i class=\"fa fa-inr\"></i>&nbsp;<label ng-bind=\"vm.billDetails.tax\"></label></td>\r" +
    "\n" +
    "                                </tr>--></tbody></table></div></div><div class=\"col-lg-12 col-md-12 col-sm-12\"><div class=\"table-responsive\"><table class=\"table table-bordered\"><thead><tr><th style=\"width:25%\">Labour Charge</th><th style=\"width:25%\">Lavy</th><th style=\"width:25%\">Transport Charge</th><th style=\"width:25%\">VAT/CST</th></tr></thead><tbody><tr class=\"warning\"><td><i class=\"fa fa-inr\"></i>&nbsp;<label ng-bind=\"vm.billDetails.labourCharge\"></label></td><td><i class=\"fa fa-inr\"></i>&nbsp;<label ng-bind=\"vm.billDetails.lavy\"></label></td><td><i class=\"fa fa-inr\"></i>&nbsp;<label ng-bind=\"vm.billDetails.transportCharge\"></label></td><td><i class=\"fa fa-inr\"></i>&nbsp;<label ng-bind=\"vm.billDetails.tax\"></label></td></tr></tbody></table></div></div></div><div class=\"ttl-amts\"><h4><strong>Bill Amount : <i class=\"fa fa-inr\"></i>&nbsp;{{vm.billDetails.grandTotal}}</strong></h4></div></div></div><div class=\"panel-footer red\"><span class=\"\"><button type=\"button\" class=\"btn btn-danger\" print-btn><i class=\"fa fa-print\"></i>&nbsp; Print</button> <button type=\"button\" class=\"btn btn-default\" ng-click=\"vm.cancelView();\"><i class=\"fa fa-reply\"></i>&nbsp; Back</button></span></div></div>"
  );
