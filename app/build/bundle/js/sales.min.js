!function() {
    ...
    function AddBillController($state, dataAccessService, alertMessage, messages) {
        function BillItem() {
            this.product = "", this.units = 1, this.quantity = 1, this.rate = 0;
        }
        function saveBill() {
        ...
        function calculate() {
            for (var grandTotal = 0, index = 0; index < vm.billDetails.billItems.length; index++) {
                var item = vm.billDetails.billItems[index];
                item.total = item.rate * item.quantity * item.units, grandTotal += item.total;
            }
        ...
        vm.addNewRow = addNewRow, vm.removeRow = removeRow, vm.calculate = calculate;
    }
