angular.module('myShopApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('customers.edit.html',
    "<form name=\"updateCustomer\" role=\"form\" novalidate ng-submit=\"updateCustomer.$valid && vm.saveCustomer()\"><div class=\"panel panel-primary\"><div class=\"panel-heading\"><h3 class=\"panel-title\"><i class=\"fa fa-pencil-square fa-fw\"></i> &nbsp; Update Customer</h3></div><div class=\"panel-body\"><div uib-alert ng-repeat=\"alert in vm.alertService.alerts\" ng-class=\"'alert-' + (alert.type || 'warning')\" close=\"vm.alertService.closeAlert($index)\" dismiss-on-timeout=\"2000\">{{alert.msg}}</div><div class=\"row\"><div class=\"col-lg-6\"><div class=\"form-group\" ng-class=\"{'has-error':updateCustomer.customerName.$dirty && updateCustomer.customerName.$invalid}\"><label>Full Name</label><input type=\"text\" class=\"form-control\" name=\"customerName\" ng-model=\"vm.customer.name\" ng-required=\"true\" ng-pattern=\"/^[a-z A-Z]+$/\"><div ng-messages=\"updateCustomer.customerName.$dirty && updateCustomer.customerName.$error\"><div ng-messages-include=\"error-messages.html\"></div></div></div><div class=\"form-group\" ng-class=\"{'has-error':updateCustomer.shopName.$dirty && updateCustomer.shopName.$invalid}\"><label>Shop Name</label><input type=\"text\" class=\"form-control\" name=\"shopName\" ng-model=\"vm.customer.shopName\" ng-pattern=\"/^[a-zA-Z]+$/\"><div ng-messages=\"updateCustomer.shopName.$dirty && updateCustomer.shopName.$error\"><div ng-messages-include=\"error-messages.html\"></div></div></div></div><div class=\"col-lg-6\"><div class=\"form-group\" ng-class=\"{'has-error':updateCustomer.contactNo.$dirty && updateCustomer.contactNo.$invalid}\"><label>Contact No.</label><input type=\"number\" class=\"form-control\" name=\"contactNo\" ng-maxlength=\"10\" ng-minlength=\"10\" ng-model=\"vm.customer.mobileNumber\"><div ng-messages=\"updateCustomer.contactNo.$dirty && updateCustomer.contactNo.$error\"><div ng-messages-include=\"error-messages.html\"></div></div></div><div class=\"form-group\" ng-class=\"{'has-error':updateCustomer.address.$dirty && updateCustomer.address.$invalid}\"><label>Address</label><textarea rows=\"3\" class=\"form-control\" name=\"address\" ng-model=\"vm.customer.address\"></textarea><div ng-messages=\"updateCustomer.address.$dirty && updateCustomer.address.$error\"><div ng-messages-include=\"error-messages.html\"></div></div></div></div></div></div><div class=\"panel-footer\"><span class=\"\"><button type=\"submit\" class=\"btn btn-primary\">Update</button> <button type=\"button\" class=\"btn btn-default\" ng-click=\"vm.cancelUpdate();\">Cancel</button></span></div></div></form>"
  );


  $templateCache.put('customers.list.html',
    "<div class=\"panel panel-primary\"><div class=\"panel-heading\"><h3 class=\"panel-title\"><i class=\"fa fa-list fa-fw\"></i>&nbsp; Customers</h3></div><div class=\"panel-body\"><div class=\"row\"><div class=\"col-lg-12\"><!--<h2>Bordered with Striped Rows</h2>--><div class=\"table-responsive\"><table class=\"table table-bordered table-hover table-striped\"><thead><tr><th>Name</th><th>Shop Name</th><th>Mobile</th><th>Address</th><th></th></tr></thead><tbody><tr ng-repeat=\"customer in customerList.customers track by customer._id\"><td ng-bind=\"customer.name\"></td><td ng-bind=\"customer.shopName\"></td><td ng-bind=\"customer.mobileNumber\"></td><td ng-bind=\"customer.address\"></td><td><button type=\"button\" class=\"btn btn-primary btn-circle\" ng-click=\"vm.editCustomer(customer._id)\"><i class=\"fa fa-pencil-square-o\" aria-hidden=\"true\"></i></button></td></tr></tbody></table></div></div></div></div></div>"
  );
